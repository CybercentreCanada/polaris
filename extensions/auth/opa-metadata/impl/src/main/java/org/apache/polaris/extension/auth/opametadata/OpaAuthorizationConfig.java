...
public interface OpaAuthorizationConfig {
  ...
  default void validate() {
    checkArgument(
        policyUri().isPresent(), "polaris.authorization.opametadata.policy-uri must be configured");

    URI uri = policyUri().get();
    String scheme = uri.getScheme();
    checkArgument(
        "http".equalsIgnoreCase(scheme) || "https".equalsIgnoreCase(scheme),
        "polaris.authorization.opametadata.policy-uri must use http or https scheme, but got: "
            + scheme);
